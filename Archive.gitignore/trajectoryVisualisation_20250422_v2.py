...
def forward_kinematics_franka(joint_angles):
    ...
    for i, (a, alpha, d, _) in enumerate(DH_PARAMS):
        theta = joint_angles[i] + joint_offsets[i]  # Add joint offset
...
def update_plot():
    ...
    fk_points = forward_kinematics_franka(joint_angles)

    # Remove previous angle labels
    ...
    # === Add angle labels beside joints ===
    for i, (x, y, z) in enumerate(fk_points):
        label = ax.text(
            x, y, z + 0.03,  # add small z-offset so text doesn't overlap line
            f"{joint_angles[i-1]:.2f}" if i > 0 else "",  # skip base, show from joint_1
